        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-green-100 shadow-lg">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-sm font-medium text-green-600">Total States/UTs</p>
                <p className="text-2xl font-bold text-green-800">{filteredPopulation.length}</p>
              </div>
            </div>
          </div>
          <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-green-100 shadow-lg">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-sm font-medium text-emerald-600">Total Districts (South India)</p>
                <p className="text-2xl font-bold text-emerald-800">{filteredWater.length}</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b-2 border-green-100">
                    <th className="text-left py-4 px-4 font-semibold text-green-800">State/UT</th>
                    <th className="text-left py-4 px-4 font-semibold text-green-800">Population</th>
                    <th className="text-left py-4 px-4 font-semibold text-green-800">Growth Rate</th>
                    <th className="text-left py-4 px-4 font-semibold text-green-800">Density</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredPopulation.length > 0 ? (
                    filteredPopulation.map((row, idx) => (
                      <tr key={idx} className="hover:bg-green-50/50 transition-colors duration-200 border-b border-green-50">
                        ...
                        <td className="py-4 px-4 text-green-700">{row["Population 2011"]}</td>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b-2 border-emerald-100">
                    <th className="text-left py-4 px-3 font-semibold text-emerald-800 min-w-32">District</th>
                    <th className="text-left py-4 px-3 font-semibold text-emerald-800 min-w-24">Canals No.</th>
                    <th className="text-left py-4 px-3 font-semibold text-emerald-800 min-w-32">Canals Length (Km.)</th>
                    <th className="text-left py-4 px-3 font-semibold text-emerald-800 min-w-24">Tube Wells</th>
                    <th className="text-left py-4 px-3 font-semibold text-emerald-800 min-w-24">Open Wells</th>
                    <th className="text-left py-4 px-3 font-semibold text-emerald-800 min-w-32">Domestic Wells</th>
                    <th className="text-left py-4 px-3 font-semibold text-emerald-800 min-w-24">Reservoirs</th>
                    <th className="text-left py-4 px-3 font-semibold text-emerald-800 min-w-20">Tanks</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredWater.length > 0 ? (
                    filteredWater.map((row, idx) => (
                      <tr key={idx} className="hover:bg-emerald-50/50 transition-colors duration-200 border-b border-emerald-50">
                        <td className="py-4 px-3 text-emerald-900 font-medium">{row["District"]}</td>
                        <td className="py-4 px-3 text-emerald-700">{row["Canals no."]}</td>
                        <td className="py-4 px-3 text-emerald-700">{row["Canals Length (Km.)"]}</td>
                        <td className="py-4 px-3 text-emerald-700">
                          {row["Tube Wells & Other Wells"]}
                        </td>
                        <td className="py-4 px-3 text-emerald-700">{row["Open Wells"]}</td>
                        <td className="py-4 px-3 text-emerald-700">
                          {row["Wells used for Domestic Purpose only"]}
                        </td>
                        <td className="py-4 px-3 text-emerald-700">{row["Reservoirs"]}</td>
                        <td className="py-4 px-3 text-emerald-700">{row["Tanks"]}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan="8" className="text-center py-12 text-emerald-500 font-medium">
                        No matching results found
                      </td>
                    </tr>
                  )}
